...
class MassReply(discord.ui.View):
  def __init__(self):
  ...
  @discord.ui.button(label='Reply', style=discord.ButtonStyle.blurple, custom_id='massreply', emoji="↩️")
  async def reply(self, interaction: discord.Interaction, button: discord.ui.Button):
    ...
    await interaction.message.edit(embeds=interaction.message.embeds, view=MassUnReply())
    def check(message):
...
class MassUnReply(discord.ui.View):
  def __init__(self):
    super().__init__(timeout=None)
    ticketbtn = Button(label="Create a Ticket", style=discord.ButtonStyle.link, url="https://discord.com/channels/717029019270381578/1083745974402420846")
    self.add_item(ticketbtn)

  @discord.ui.button(label='Reply', style=discord.ButtonStyle.grey, custom_id='massunreply', emoji="↩️", disabled=True)
...
class MassDm(commands.Cog):
  ...
  @app_commands.command(
    name = "mass-dm",
    description = "Mass DM lots of users"
  )
  ...
  @app_commands.describe(
    users = "User IDs separated with commas",
  ...
  async def massdm(
    ...
    for user in users.split(","):
      userObject = interaction.guild.get_member(int(user.strip()))
      ...
      try:
        ...
        await userObject.send(embed=embed, view=MassReply())
        
        chn = interaction.client.get_channel(1270550367335350385)
        embed = discord.Embed(title="DM Sent", color=0x41ff00)
        embed.add_field(name="__Member__", value=f"**User Mention:** {userObject.mention}\n**User Name:** {userObject.name}\n**User ID:** `{userObject.id}`", inline=True)
        ...
        print(e)
        embed = discord.Embed(description=f"Unable to DM {user.mention}.\n```{e}```")
        embeds.append(embed)
