...
class Sticky(commands.GroupCog, name="sticky"):
  ...
  async def sticky_enable(
    ...
    await channel.edit(topic=str(channel.topic)+" *Sticky Message Enabled. Do NOT delete/edit this message.*")
  ...
  async def sticky_disable(
    ...
    if found:
      ...
      embed.timestamp = datetime.datetime.utcnow()
      await interaction.response.send_message(embed=embed, ephemeral=True)
      await channel.edit(topic=str(channel.topic).replace("*Sticky Message Enabled. Do NOT delete/edit this message.*", " "))
    else:
