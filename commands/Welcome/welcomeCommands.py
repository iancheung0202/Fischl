...
class embed_modal(discord.ui.Modal, title = "Setup Embed Welcome Message"):
  ...
  msg = discord.ui.TextInput(label="Normal message content", style=discord.TextStyle.paragraph, placeholder="Head over to tinyurl.com/fischl-var for available variables", max_length=2000, required=False)
  
  embedtitle = discord.ui.TextInput(label="Title of the embed", style=discord.TextStyle.paragraph, placeholder="Head over to tinyurl.com/fischl-var for available variables", max_length=256, required=False)
  
  description = discord.ui.TextInput(label="Description of the embed", style=discord.TextStyle.paragraph, placeholder="Head over to tinyurl.com/fischl-var for available variables", max_length=4000, required=False)
  ...
  async def on_submit(self, interaction:discord.Interaction):
    ...
    embed = discord.Embed(title="✅ Welcome message enabled!", description=f'Congratulations! Now the bot will send a welcome message to <#{welcomeChannel}> every time a new member joins the server!\n\n**To view a sample welcome message, use </welcome sample:1043590008667385876>.**', colour=0x00FF00)
...
class Welcome(commands.GroupCog, name="welcome"):
  ...
  @app_commands.command(
    name = "setup",
    description = "Initalize welcome function in the server"
  )
  ...
  @app_commands.command(
    name = "disable",
    description = "Disable welcome function in the server"
  )
  @app_commands.checks.has_permissions(administrator=True)
  async def welcome_disable(
    ...
    if found:
      embed = discord.Embed(title="Welcome message disabled!", description=f'Sorry to see you go, but if you change your mind at anytime, use </welcome setup:1043590008667385876> again.', colour=0xFF0000)
      embed.timestamp = datetime.datetime.now(datetime.timezone.utc)
      await interaction.response.send_message(embed=embed, ephemeral=True)
    else:
      embed = discord.Embed(title="Welcome message not enabled!", description=f'Welcome function has never been enabled, or it has already been disabled. ', colour=0xFF0000)
      embed.timestamp = datetime.datetime.now(datetime.timezone.utc)
      await interaction.response.send_message(embed=embed, ephemeral=True)
  ...
  @app_commands.command(
    name = "sample",
    description = "Sends a sample welcome message"
  )
  @app_commands.checks.has_permissions(administrator=True)
      ...
      for key, val in welcomecontent.items():
        if val['Server ID'] == interaction.guild.id:
          if (val['Title'] != "" or val['Description'] != ""):
            ...
            embed = discord.Embed(title=script(val['Title'], interaction.user, interaction.guild), description=script(val['Description'], interaction.user, interaction.guild), color=color)
            if welcomeImageEnabled:
              filename = await createWelcomeMsg(interaction.user, bg=f"./assets/Welcome Image Background/{interaction.guild.id}.png")
        ...
        # break
          ...
          await channel.send(script(val['Message Content'], interaction.user, interaction.guild), embed=embed, file=file)
